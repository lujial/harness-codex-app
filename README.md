...
- **静态后台插件**（`codex-plugin/comfyui-static-plugin/`）：把 AI 生图 / ComfyUI 嵌入 / 额度查询做成 **profile 静态 bundle**（`dsh plugin --profile web add` 安装），服务启动自动加载、页面刷新自动注入 —— **关闭页面后功能仍在**，无需每次 `cordis_run`
...
│   └── comfyui-static-plugin/ 静态后台插件（profile bundle，重启/刷新不丢失）
│       ├── lib/index.js       Host：/comfy-api/* 路由（生图/模型/余额）+ /gen-img
│       ├── lib/client.js      Client：__ModuleLoader__.load bundle（设置页 + 全屏）
│       ├── cordis.patch.yml   组合补丁：插入 comfyui-static 行
│       └── package.json       dsh.bundle / dsh.client 声明
├── screenshots/               界面预览图
├── start-dsh.ps1              后台运行 Harness 服务的脚本（可选）
├── restart-dsh.ps1            重启 Harness 服务（安装静态插件后生效）
└── README.md
```
