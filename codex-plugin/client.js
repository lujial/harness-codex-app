...
return {
  apply(ctx) {
    ...
    function SessionView(props) {
      ...
      React.useEffect(() => {
        let alive = true
        host.call('models').then((r) => { if (alive) setModels(r) }).catch(() => {})
        host.call('plan', { sessionId }).then((r) => { if (alive && r) setPlanActive(!!r.active) }).catch(() => {})
        return () => { alive = false }
      }, [sessionId])
      ...
      const saveModel = (provider, model) => {
        host.call('saveModel', { provider, model }).then((r) => {
      ...
      const togglePlan = () => {
        ...
        setPlanActive(next)
        host.call('planSet', { sessionId, active: next }).catch(() => {})
      }
      const stop = () => {
